...
config.keys = {
  ...
  -- Restore default session on startup
  {
    key = 'R',
    mods = 'CTRL|SHIFT|ALT',
...
-- ============================================================================
-- Apply the sessions plugin with auto-restore enabled
sessions.apply_to_config(config)

-- Session persistence configuration
-- This will automatically save your session and restore it on startup
config.enable_kitty_keyboard = false  -- Prevents conflicts with some session features

-- Add event handler for automatic session restore on startup
wezterm.on('gui-startup', function(cmd)
  -- Automatically restore the last saved session
  local success, stdout = wezterm.run_child_process({ 'ls', wezterm.home_dir .. '/.local/share/wezterm/sessions' })
  if success then
    wezterm.emit('restore_session')
  end
end)
